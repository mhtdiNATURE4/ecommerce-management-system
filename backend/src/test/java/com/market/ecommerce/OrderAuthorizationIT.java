...
@ActiveProfiles("test")
public class OrderAuthorizationIT {
    ...
    @Test
    void customerCanAccessOwnOrderButNotAnotherCustomerOrder() {
        Long orderId = checkoutAs(tokenA, addrAId);

        ResponseEntity<String> ownerResponse = requestGetOrder(orderId, tokenA);
        assertThat(ownerResponse.getStatusCode()).isEqualTo(HttpStatus.OK);

        ResponseEntity<String> otherResponse = requestGetOrder(orderId, tokenB);
    ...
    void adminCanAccessAnyOrder() {
        ...
        ResponseEntity<String> adminResponse = requestGetOrder(orderId, tokenAdmin);
    ...
    void anotherCustomerCannotCancelOrder() {
        ...
        ResponseEntity<String> cancelResponse = requestCancelOrder(orderId, tokenB);
    ...
    private ResponseEntity<String> requestGetOrder(Long orderId, String token) {
        return restTemplate.exchange(url("/api/orders/" + orderId), HttpMethod.GET, new HttpEntity<>(authHeaders(token)), String.class);
    }
