-- V9: replace legacy seed products with the new Electronics and Books catalog

-- Remove cart references to the legacy seeded products first so their rows can be deleted safely.
DELETE FROM cart_items
WHERE product_id IN (
    SELECT id FROM products WHERE name IN ('Wireless Mouse', 'Clean Code', 'Classic T-Shirt')
);

-- Remove order references to the legacy seeded products so their rows can be deleted safely.
DELETE FROM order_items
WHERE product_id IN (
    SELECT id FROM products WHERE name IN ('Wireless Mouse', 'Clean Code', 'Classic T-Shirt')
);

-- Remove the legacy seeded products.
DELETE FROM products
WHERE name IN ('Wireless Mouse', 'Clean Code', 'Classic T-Shirt');

-- Remove the unused Clothing category if no products still reference it.
